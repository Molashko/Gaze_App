...
EYE_DRAW_KEYS = ("inner", "outer", "top_lid", "bottom_lid")
...
def _draw_text(
    ...
    # Если Pillow недоступен, рисуем стандартным OpenCV-шрифтом.
    if not PIL_AVAILABLE:
        cv2.putText(img, text, org, cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, thickness)
        return

    # Через Pillow текст обычно выглядит аккуратнее, особенно для кириллицы.
...
class GazeApp:
    def __init__(self) -> None:
        # Инициализируем состояние приложения и параметры калибровки.
        ...
        # Основное окно управления (Tk), отдельные окна отображения — через OpenCV.
        self.root = tk.Tk()
    ...
    def _get_screen_size(self) -> Tuple[int, int]:
        try:
            # На Windows учитываем DPI-aware режим, чтобы пиксели были физическими.
            ctypes.windll.user32.SetProcessDPIAware()
            w = ctypes.windll.user32.GetSystemMetrics(0)
            h = ctypes.windll.user32.GetSystemMetrics(1)
            return int(w), int(h)
        except Exception:
            # Резервный способ через tkinter, если WinAPI недоступен.
            tmp = tk.Tk()
            tmp.withdraw()
    ...
    def _create_calibration_points(self) -> List[Tuple[int, int]]:
        # Плотная сетка + дополнительные крайние точки для лучшего покрытия экрана.
        grid_points = [
            (int(x_pct * self.screen_w), int(y_pct * self.screen_h))
        ...
        return list(dict.fromkeys([*grid_points, *extra_points]))
    ...
    def _ensure_mediapipe(self) -> bool:
        # Единая точка проверки зависимости перед запуском трекинга/калибровки.
        if MEDIAPIPE_AVAILABLE:
            return True
        messagebox.showerror("Ошибка", "MediaPipe не установлен")
        return False

    def _close_face_mesh(self) -> None:
        # Безопасное закрытие MediaPipe-объекта.
    ...
    def _open_camera(self):
        # Открываем камеру и выставляем параметры потока из config.
    ...
    def _setup_windows() -> None:
        # Настраиваем окна вывода: полноэкранное окно взгляда + отладочное окно камеры.
    ...
    @staticmethod
    def _show_warmup(frame: np.ndarray, display_w: int, display_h: int) -> None:
        # Пока камера стабилизируется, не делаем вычисления трекинга.
    ...
    @staticmethod
    def _draw_text_lines(canvas: np.ndarray, lines: List[Tuple[str, Tuple[int, int], float, Tuple[int, int, int], int]]) -> None:
        # Утилита для компактного вызова нескольких строк _draw_text.
        for text, pos, scale, color, thickness in lines:
            _draw_text(canvas, text, pos, font_scale=scale, color=color, thickness=thickness)

    @staticmethod
    def _draw_eye_debug(cam_display: np.ndarray, landmarks, img_w: int, img_h: int) -> None:
        # Визуализация ключевых точек век и центра радужки для обоих глаз.
        for eye_dict, color in ((RIGHT_EYE, (0, 255, 0)), (LEFT_EYE, (0, 255, 0))):
            for key in EYE_DRAW_KEYS:
    ...
    def _process_face(self, frame: np.ndarray, img_w: int, img_h: int):
        # Полный шаг обработки лица на кадре:
        # детект FaceMesh -> head pose -> признаки взгляда -> debug-отрисовка.
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = self.face_mesh.process(rgb_frame)
        cam_display = frame.copy()
        if not results.multi_face_landmarks:
            # На кадре нет лица: отдаем пустые признаки и предупреждение на превью.
            _draw_text(
                cam_display,
                "Лицо не обнаружено!",
                (10, 25),
        ...
        if gaze_features:
            self._draw_text_lines(
                ...
                [
                    ...
                    (f"Голова: Y={yaw:.0f} P={pitch:.0f}", (10, 50), 0.5, (255, 200, 0), 1),
                ],
            )
        return cam_display, gaze_features

    def _handle_calibration(self, canvas: np.ndarray, gaze_features) -> None:
        # Логика пошаговой калибровки по текущей точке.
        if not (self.calibrating and not self.calibration_complete):
            return
        if self.current_calib_idx >= len(self.calib_points):
            # Все точки пройдены — финализируем модель.
        ...
        if self.calib_point_start is None:
            # Старт таймера удержания текущей калибровочной точки.
        ...
        if gaze_features is not None and elapsed > self.calib_hold_time * CALIBRATION_WARMUP_RATIO:
            # После короткого "прогрева" точки копим признаки для устойчивой медианы.
        ...
        if progress > 0:
            cv2.ellipse(canvas, (cx, cy), (outer_r, outer_r), -90, 0, int(360 * progress), (0, 255, 0), 6)
        cv2.circle(canvas, (cx, cy), 25, (255, 255, 255), -1)
        cv2.circle(canvas, (cx, cy), 8, (0, 0, 255), -1)
        self._draw_text_lines(
            canvas,
            [
                (f"Калибровка: {self.current_calib_idx + 1}/{len(self.calib_points)}", (10, 40), 1.0, (255, 255, 255), 2),
                ("СМОТРИТЕ НА КРАСНУЮ ТОЧКУ", (10, 85), 0.9, (0, 200, 255), 2),
                (f"Образцы: {len(self.calib_samples)}/{self.calib_min_samples}", (10, 125), 0.7, (180, 180, 180), 2),
            ],
        )

        if elapsed >= self.calib_hold_time and len(self.calib_samples) >= self.calib_min_samples:
            # Для каждой точки берем медианный вектор и сохраняем как калибровочный сэмпл.
            median_feature = np.median(np.array(self.calib_samples, dtype=np.float64), axis=0)
            self.tracker.add_calibration_sample(median_feature, (cx / self.screen_w, cy / self.screen_h))
            self.current_calib_idx += 1
            self.calib_point_start = None
            self.calib_samples.clear()

    def _draw_prediction(self, canvas: np.ndarray, gaze_features, display_h: int) -> None:
        # Предсказание точки взгляда и отрисовка маркера.
        if gaze_features is None or not self.tracker.calibrated:
            return
        feature_vec = self.tracker.build_feature_vector(gaze_features, self.current_head_pose)
        ...
        # Ограничиваем координаты экраном, затем сглаживаем траекторию.
        screen_x, screen_y = np.clip(screen_pos, [0.0, 0.0], [self.screen_w - 1, self.screen_h - 1])
        dot_x, dot_y = map(int, self.tracker.smooth(screen_x, screen_y))
        cv2.circle(canvas, (dot_x, dot_y), 20, (0, 0, 255), -1)
        cv2.circle(canvas, (dot_x, dot_y), 22, (255, 255, 255), 3)
        if self.calibrating:
            # Во время калибровки не захламляем экран дополнительными подсказками.
            return
        ...
        self._draw_text_lines(
            canvas,
            [
                (f"Позиция: ({dot_x}, {dot_y})", (10, 40), 1.0, (255, 255, 255), 2),
                ("Смотрите на круги | Нажмите Q для выхода", (10, display_h - 25), 0.6, (100, 100, 100), 1),
            ],
        )

    def _build_ui(self) -> None:
        # Компактная панель управления калибровкой/запуском.
    ...
    def _run_calibration(self) -> None:
        # Запускаем калибровку, полностью сбрасывая предыдущее состояние.
    ...
    def _start_tracking(self) -> None:
        ...
        while self.tracking_active:
            ...
            if frame_count < WARMUP_FRAMES:
                self._show_warmup(frame, display_w, display_h)
                continue

            # Шаг анализа лица и визуализация на окне камеры.
            cam_display, gaze_features = self._process_face(frame, img_w, img_h)
            cv2.imshow(WINDOW_CAMERA, cam_display)
            canvas = np.zeros((display_h, display_w, 3), dtype=np.uint8)

            # Шаги бизнес-логики: калибровка и/или предсказание.
            self._handle_calibration(canvas, gaze_features)
            self._draw_prediction(canvas, gaze_features, display_h)

            cv2.imshow(WINDOW_GAZE, canvas)

            try:
                # Поддерживаем отзывчивость Tk-интерфейса во время OpenCV-цикла.
        ...
        # Корректно освобождаем ресурсы после выхода из цикла.
    ...
    def _reset_calibration(self) -> None:
        # Сброс только калибровки (без закрытия приложения).
        self.calibrating = False
        self.calibration_complete = False
        self.current_calib_idx = 0
        self.tracker.reset()
    ...
    def _quit(self) -> None:
        # Полный выход из приложения и освобождение ресурсов.
        self.tracking_active = False
